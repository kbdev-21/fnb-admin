...
export default function ProductCard({ product }: { product: Product }) {
    ...
    return (
        <>
            <div
                className={`group flex flex-col gap-3 rounded-4xl border border-border bg-card overflow-hidden transition-all duration-300 relative ${
            ...
            >
                {isUnavailable && (
                    <>
                        <div className="absolute inset-0 bg-card/40 z-10 pointer-events-none" />
                        <div className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
                            <div className="bg-gray-200 text-primary px-3 py-2 rounded-full font-[700] text-sm shadow-lg">
                                Sold Out
                            </div>
                        </div>
                    </>
                ...
                <div className="flex flex-col gap-2 px-4 pb-4 text-center">
                    <h3
                        className={`text-md font-semibold line-clamp-2 text-primary transition-colors ${
                            isUnavailable ? "" : "group-hover:underline"
                        }`}
                    >
                        {product.name}
                    </h3>
...
function UpdateProductAvailabilityDialog({
    ...
    useEffect(() => {
        if (productQuery.data && authStoreCode) {
            ...
            const available =
                !productQuery.data.unavailableAtStoreCodes.includes(
                    authStoreCode
                );
